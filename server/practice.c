...
void handle_begin_practice(int socket_fd, int user_id, int room_id)
{
    ...
    // Lấy câu hỏi và append vào response
...
void submit_practice(int socket_fd, int user_id)
{
  ...
  pthread_mutex_unlock(&server_data.lock);
  // Compute actual time taken (in minutes)
int time_taken = 0;
{
    const char *sql_time = 
        "SELECT (strftime('%s', COALESCE(end_time, DATETIME('now'))) - strftime('%s', start_time)) "
        "FROM practice_sessions WHERE id=? LIMIT 1;";
    
    sqlite3_stmt *tstmt;

    if (sqlite3_prepare_v2(db, sql_time, -1, &tstmt, NULL) == SQLITE_OK) {
        sqlite3_bind_int(tstmt, 1, session_id);
        
        if (sqlite3_step(tstmt) == SQLITE_ROW) {
            if (sqlite3_column_type(tstmt, 0) != SQLITE_NULL) {
                long long secs = sqlite3_column_int64(tstmt, 0);
                
                if (secs < 0) secs = 0; 
                
                time_taken = (int)((secs + 30) / 60); 
            }
        }
  ...
  {
    char details[128];
    snprintf(details, sizeof(details), "Submitted practice session %d: %d/%d", session_id, score, total);
    log_activity(user_id, "PRACTICE_SUBMIT", details);
  }
}